...
export const paginationHtml = (parent, count) => {
  const paginationElem = document.createElement('div');
  paginationElem.classList.add('pagination');

  const listItems = '<li class="pagination__bar-item"></li>'.repeat(count);
  ...
  const child = `
    ...
    <div class="pagination__arrows">
      <button class="pagination__btn pagination__btn_left" type="button">
      ...
      <button class="pagination__btn pagination__btn_right" type="button">
        <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
  ...
  paginationElem.append(container(child, 'pagination__container'));
  parent.append(paginationElem);

  document.querySelector('.pagination__bar-item').classList.add('pagination__bar-item_active')
